...
const dialingCodeLookup = require('../dialing.json');
...
const decompose = function (raw, number, regionCode, mask) {
  ...
  const normal = nationalSignificantNumber ? _.compact([nationalSignificantNumber, extension]).join('x') : raw;
...
const ensureCountryDialingCode = (string, countryCode) => {
  if (string.match(/^\+/)) {
    return string;
  }
  const dialingCode = dialingCodeLookup[countryCode];
  if (!dialingCode) {
    return string;
  }
  return `+${dialingCode}${string}`
}
...
module.exports = {
  ...
  ensureCountryDialingCode: ensureCountryDialingCode,
  maskable: false,
  operators: [
    'is equal to',
    'is not equal to',
