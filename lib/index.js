  ...
  if (obj instanceof Array) {
    ...
    if ((obj.masked === false) && (obj instanceof String || obj instanceof Boolean || obj instanceof Number || obj instanceof Date)) {
      ...
      for (const key in obj) {
        ...
        obj[key] = mask(value, isMasked(obj) || doMask);
      }
      if (isMasked(obj)) { obj.masked = true; }
    }
...
function isMasked (obj) {
  // Original logic
  // return (obj.masked != null);

  // If mask is undefined or null
  if (obj.masked == null) {
    return false;
  } else {
    if (isRichTypeObject(obj.masked) && ([true, false, 'true', 'false'].includes(obj.masked.raw))) {
      return types.boolean.parse(obj.masked.raw).valueOf();
    } else {
      return true;
    }
  }
